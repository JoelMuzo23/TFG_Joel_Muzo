...
if(isset($_GET['id'])) {
    ...
    $detalles_reserva = $proyecto->detallesPagoHote($id_reserva);
    ...
    if($detalles_reserva) {
        ...
        $pdf->Cell(0, 10, 'Nombre del viaje: ' . $detalles_reserva['nombre'], 0, 1, 'L');
        $pdf->Cell(0, 10, 'Fecha de Salida: ' . $detalles_reserva['fecha_salida'], 0, 1, 'L');
        $pdf->Cell(0, 10, 'Fecha de Llegada: ' . $detalles_reserva['fecha_llegada'], 0, 1, 'L');
        $pdf->Cell(0, 10, 'Numero Viajeros: ' . $detalles_reserva['numero_viajeros'], 0, 1, 'L');
        $pdf->Cell(0, 10, 'Numero Habitaciones: ' . $detalles_reserva['numero_habitaciones'], 0, 1, 'L');
        $pdf->Ln(5);
        $pdf->SetFont('helvetica', '', 12);
        $pdf->MultiCell(0, 10, 'Por favor, completar el pago de tu reserva utilizando la siguiente información:', 0, 'L');
        $pdf->Ln(5);
        $pdf->SetFont('helvetica', 'B', 12);
        $pdf->Cell(0, 10, 'Total a pagar: ' . $detalles_reserva['precio'] * $detalles_reserva['numero_viajeros'] . ' €' , 0, 1, 'L');
        ...
        $pdf->Output('formulario_pago.pdf', 'I');
    } else {
