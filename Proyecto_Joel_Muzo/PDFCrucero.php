...
if(isset($_GET['id'])) {
    ...
    $detalles_reserva = $proyecto->detallesPagoCru($id_reserva);
    ...
    if($detalles_reserva) {
        ...
        $pdf->Cell(0, 10, 'Fecha de Salida: ' . $detalles_reserva['fecha'], 0, 1, 'L');
        $pdf->Cell(0, 10, 'Numero Viajeros: ' . $detalles_reserva['numero_viajeros'], 0, 1, 'L');
        $pdf->Cell(0, 10, 'Numero Camarotes: ' . $detalles_reserva['numero_camarotes'], 0, 1, 'L');
        $pdf->Ln(5);
        $pdf->SetFont('helvetica', '', 12);
