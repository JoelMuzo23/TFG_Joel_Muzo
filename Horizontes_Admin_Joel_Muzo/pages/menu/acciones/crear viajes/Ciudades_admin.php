...
<head>
    ...
    <link rel="stylesheet" href="../../../../estilos/Ciudades_admin.css">
...
<body>
    ...
    <div class="parallax">
        ...
        <div class="content">
            ...
            <h1 class="display-4 fw-bold">Viajes a ciudades</h1>
            <!--Encabezado secundario del fondo--> 
            <h3 class="display-8 fw-bold">Viajes a ciudades: Administración Central</h3>
        </div>
    </div>
        ...
        if(isset($_SESSION["nombre"])){
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                if(!empty($_POST["descripcion"])){
                    ...
                    $error[] = "Por favor,  introduzca una descripcion de la ciudad";
                }
                ...
                if(!empty($_POST["transporte"])){
                    ...
                    $error[] = "Por favor,  introduzca los transporte de la ciudad";
                }
                ...
                if(!empty($_FILES["imagen"]["name"]) && !empty($_FILES["imagen1"]["name"]) && !empty($_FILES["imagen2"]["name"]) && !empty($_FILES["imagen3"]["name"])){
                    ...
                    $imagen_ciudad = file_get_contents($imagen);
                    $imagen_ciudad1 = file_get_contents($imagen1);
                    $imagen_ciudad2 = file_get_contents($imagen2);
                    $imagen_ciudad3 = file_get_contents($imagen3);
                }else{
                ...
                if(empty($error)){
                    ...
                    $insertar -> insertarViajeCiu($nombre, $pais, $descripcion, $transporte, $informacion, $precio, $imagen_ciudad, $imagen_ciudad1, $imagen_ciudad2, $imagen_ciudad3);
                }
    ...
    <!-- 
        Si el usuario ha iniciado sesion, se va a mostrar un texto descriptivo seguido se mostrara un formulario para insertar un nuevo viaje de la tematica de ciudades,
        este formulario se valida y se limpia de caracteres malisiosos y seguido dichos valores introducidos por el admin se agregaran a la funcion insertarViajeCiu(),
        que basicamente va a insertar dicho viaje a la base de datos.
    -->
    <div class="container mt-4">
        <div class="row">
            <div class="col-md-8 offset-md-2">
                <h2>Introduce los datos de la ciudad</h2>
                <form action="<?=htmlspecialchars($_SERVER["PHP_SELF"])?>" method="post" enctype="multipart/form-data">
                    <div class="form-group">
                        <label for="nombre">Introduce el nombre de la ciudad</label>
                        <input type="text" class="form-control" id="nombre" name="nombre" value="<?= htmlspecialchars($_POST["ciudad"] ?? '')?>">
                    </div>
                    ...
                    <div class="form-group mt-3">
                        <label for="descripcion">Introduce una descripcion de la ciudad</label>
                        <textarea class="form-control" id="descripcion" name="descripcion" rows="3"><?= htmlspecialchars($_POST["descripcion"] ?? '')?></textarea>
                    </div>
                    <div class="form-group mt-3">
                        <label for="transporte">Introduce los transporte  de la ciudad</label>
                        <textarea class="form-control" id="transporte" name="transporte" rows="2"><?= htmlspecialchars($_POST["transporte"] ?? '')?></textarea>
                    </div>
                    <div class="form-group mt-3">
                        <label for="informacion">Introduce la informacion de la ciudad</label>
                    ...
                    <div class="form-group mt-3">
                        <label for="pais">Inserta la imagen del viaje</label>
